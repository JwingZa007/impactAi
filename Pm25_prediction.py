...
for i, step in enumerate(prediction_steps):
    ...
    start_idx, end_idx = len(data_before) + step, len(data_before) + step + 6

    if end_idx <= len(data):
        real_avg = data.iloc[start_idx:end_idx][features].values.mean()
        print(f"真實 {hours}h{minutes:02d}min 平均 PM2.5 = {real_avg:.2f} (預測 {y_pred_real[i]:.2f})")
    else:
